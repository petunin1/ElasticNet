...
t0 = now()
beta = elasticNet(x, y, l1 = l1, l2 = l2, cholesky = False)
...
t0 = now()
en = ElasticNet(alpha = l1 / 2 + l2, l1_ratio = l1 / 2 / (l1 / 2 + l2), fit_intercept = False, max_iter = 1000);
en.fit(x, y)
beta2 = en.coef_
print('sklearn.linear_model.ElasticNet latency:    {:.0f}ms'.format(now() - t0))
